...
def build_xceptionfuzzyFC_model(input_shape=(128, 128, 3), num_classes=3):
    ...
    for layer in base_model.layers[:-10]:  # Freeze các layer đầu
    ...
    x = layers.Dense(128, activation='gelu',
                        kernel_regularizer=tf.keras.regularizers.l2(0.05))(x) # MUST 128!!! (1e-4)
    x = layers.BatchNormalization()(x)
...
def train_xceptionfuzzyFC(X_train, y_train, X_val, y_val):
    model = build_xceptionfuzzyFC_model()
